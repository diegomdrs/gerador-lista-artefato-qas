...
module.exports = class Gerador {
  ...
  obterListaCommitArquivo(saida, nomeProjeto) {
    ...
    return listaSaidaTask.reduce((accum, saidaTask) => {
      ...
      const commitMensagem = saidaTask.match(/[^\r\n]+/g)[0]

      // const numeroTarefa = commitMensagem.match(/task.*\d-/i)[0].match(/\d+/)[0]
      const numeroTarefa = commitMensagem.match(/^[^\d]*(\d+)/i)[0].match(/\d+/)[0]
      const descricaoTarefa = commitMensagem.split(numeroTarefa)[1].trim()

      const listaArquivo = saidaTask.match(/[^\r\n]+/g).slice(1)

      accum.push.apply(accum,
        listaArquivo.map(arquivo => new Arquivo(nomeProjeto, numeroTarefa, descricaoTarefa, arquivo)))
